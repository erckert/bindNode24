...
class FileSetter(object):
    ...
    def embeddings_input():
        return '/home/frank/bind/bindPredict/embd_half_trimmed.h5'
        # return '/home/frank/bind/bindPredict/embed_batches/missing.h5'
    ...
    @staticmethod
    def embeddings_and_structures_input():
        return '/home/frank/bind/bindPredict/dssp8.h5'
        #return "/home/frank/bind/bindPredict/embd_ankh.h5"
        # old version was: /home/frank/bind/bindPredict/embd_af2.h5
        # currently set to new version, having 1310 ids

    @staticmethod
    def predictions_folder():
        return '/home/frank/bind/bindPredict/predictions'  # TODO set path to where predictions should be written
    ...
    @staticmethod
    def split_ids_in():
        return 'data/development_set/ids_dssp_split'  # cv splits used during development; available on GitHub

    @staticmethod
    def test_ids_in():
        return 'data/development_set/uniprot_test_dssp.txt'  # test ids used during development; available on GitHub
...
class FileManager(object):
    ...
    @staticmethod
    def read_af2arr(header):
        """
        read the af2 arrays and mean along min/max
        """
        af2arr = np.load(f"/home/frank/bind/bindPredict/data/afstruc/{header}.npy")
        return np.mean(af2arr, axis=2)
    ...
    @staticmethod
    def read_embeddings_and_structures(file_in):
        """
        Read embeddings and structures from .h5-file.
        :param file_in:
        :return: dict with key: ID, value: per-residue embeddings
        """
        ...
        structures = dict()
        with h5py.File(file_in, 'r') as f:
            for key, concatarr in f.items():
                embeddings[key] = numpy.array(concatarr[:,:1024], dtype=numpy.float32)
                structures[key] = numpy.array(concatarr[:,1024:], dtype=numpy.float32)
                
        assert len(embeddings)==len(structures), "Corrupted read-in for combined embd/struc .h5"
        return embeddings, structures


    @staticmethod
    def read_embeddings_for_GCN_inference(file_in):
        """
        Read embeddings from .h5-file for inference with GCNConv. Create diagonal matrix as structure dummy.
        """
        embeddings = dict()
        structures = dict()
        with h5py.File(file_in, 'r') as f:
            for key, embedding in f.items():
                embeddings[key] = numpy.array(embedding[:,:1024], dtype=numpy.float32)
                structures[key]=numpy.eye(embedding.shape[0], dtype=numpy.float32)
        return embeddings, structures
