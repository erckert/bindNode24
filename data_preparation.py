...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, samples, embeddings, seqs, labels, max_length, structures, protein_prediction=False):
        ...
        self.n_features = self.get_embedding_dimensions()   # this was just a complicated way to get the embedding length. 
        self.dim_distogram = self.get_distogram_dimensions()
        self.input_dim = self.get_input_dimensions()
        print(f'Number of embedding input features: {self.n_features}, distogram_dimension {self.dim_distogram}, input dimension: ({self.input_dim+1},{self.max_length})')
    ...
    def __getitem__(self, item):
        ...
        structure = self.structures[prot_id]
        ...
        features = np.zeros((self.input_dim + 1, self.max_length), dtype=np.float32)   # shape is 1838, 813
        features[:self.n_features, :prot_length] = np.transpose(embedding)  # set feature maps to embedding values
        
        features[self.n_features:(self.n_features+structure.shape[0]), :prot_length] = normalize(np.transpose(structure))    # for now a simple stacking
        features[self.input_dim, :prot_length] = 1  # set last element to 1 because positions are not padded
    ...
    def get_embedding_dimensions(self):
        ...
        return np.shape(first_embedding)[1] # will most likely always return 1024

    def get_distogram_dimensions(self):
        # first_key = list(self.structures.keys())[0]
        # first_distogram = self.embeddings[first_key]
        
        # return np.shape(first_distogram)[1] # will most likely always return 813
        return 813

    def get_input_dimensions(self):
        return self.n_features+self.dim_distogram
...
class ProteinInformation(object):
    ...
    def get_data(ids):
        ...
        max_length = 813    # fix this here, because af2 structures have to add a fixed dimension to 1024 feature dim
    ...
    @staticmethod
    def get_labels_for_overlap(ids, sequences, file_prefix=None):
        """
        Read binding residues for metal, nucleic acids, and small molecule binding
        :param ids:
        :param sequences:
        ...
        for prot_id in ids:
            ...
            labels[prot_id] = binding_tensor
        return labels, metal_residues, nuclear_residues, small_residues
...
class ProteinResults(object):
    ...
    def set_predictions(self, predictions):
        self.predictions = np.around(np.array(predictions), 3)
    ...
    def calc_performance_measurements_in_window(self, cutoff):
        performance = self.calc_performance_given_labels_in_window(cutoff, self.labels)
    ...
    def calc_performance_given_labels(self, cutoff, ligand_labels):
        ...
        if num_ligands > 1:  # ligand-type assessment
            ...
            for i in range(0, num_ligands):
                tp = fp = tn = fn = 0
                cross_pred = [0, 0, 0, 0]
                for idx, lig in enumerate(ligand_labels):   # shape here is (len_protein, 3) containing truth labels
    ...
    def calc_performance_given_labels_in_window(self, cutoff, ligand_labels):
        """Calculate performance values for this protein"""
        performance = defaultdict(dict)
        num_ligands = np.shape(ligand_labels)[1]
            ...
            for i in range(0, num_ligands):
                ...
                for idx, lig in enumerate(ligand_labels):   # shape here is (len_protein, 3) containing truth labels
                    if self.predictions[idx, i] >= cutoff:  # predicted as binding to this ligand
                        ...
                        for j in range(0, num_ligands):
                            if i == j:  # same as predicted ligand
                                if (idx+1)<ligand_labels.shape[0]:
                                    if (lig[j] >= self.bind_cutoff) or (ligand_labels[idx+1, j]>=self.bind_cutoff) or (ligand_labels[idx-1, j]>=self.bind_cutoff):  # also annotated to this ligand
                                        tp += 1
                                        cross_pred[i] += 1
                                        true_prediction = True
                                elif (idx+1)==ligand_labels.shape[0]:
                                    if (lig[j] >= self.bind_cutoff) or (ligand_labels[idx-1, j]>=self.bind_cutoff):  # also annotated to this ligand
                                        tp += 1
                                        cross_pred[i] += 1
                                        true_prediction = True
                                else:
